...
// import associate2 from "@/assets/sponsor/associatepartner/shiv-power.png";
  ...
  return (
    <div className="group relative h-24 sm:h-28 rounded-2xl bg-white grid place-items-center px-6 transition-transform hover:scale-105 shadow-card-elevated border border-navy-deep/10">
      {logo ? (
        ...
        <span className="font-display text-2xl sm:text-3xl text-navy-deep tracking-wider">{name}</span>
      )}
      {sub && <span className="absolute bottom-2 text-[10px] tracking-[0.3em] text-navy/70">{sub}</span>}
    </div>
  );
}
...
export function Sponsors() {
  return (
    <section id="sponsors" className="section-light relative py-24 sm:py-32 overflow-hidden">
      {/* Confetti streaks */}
      ...
      <div className="container-page relative">
        ...
        <div className="text-center">
          <p className="text-xs sm:text-sm tracking-[0.4em] font-bold text-navy-deep">CO POWERED BY</p>
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-5 max-w-5xl mx-auto">
            <LogoTile name="" logo={copoweredby4} />
            <LogoTile name="" logo={copoweredby1}/>
            <LogoTile name="" logo={copoweredby2} />
            <LogoTile name="" logo={copoweredby3} />
          </div>
        </div>
          ...
          {([
            ...
            return (
              <div key={p.n} className="text-center">
                ...
                {href ? (
                  <a href={href} target="_blank" rel="noopener noreferrer" aria-label={p.n}>
                    <LogoTile name={p.n} logo={p.logo} />
                  </a>
                ) : (
                  <LogoTile name={p.n} logo={p.logo} />
